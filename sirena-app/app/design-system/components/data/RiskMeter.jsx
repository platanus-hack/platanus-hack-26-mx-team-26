...
const CSS = `
.sr-risk{ position:relative; display:flex; flex-direction:column; align-items:center; font-family:var(--font-body); }
.sr-risk__svg{ position:relative; display:block; }
.sr-risk__track{ stroke:var(--surface-inset); }
.sr-risk__glow{ opacity:.5; }
...
.sr-risk__ends{ display:flex; justify-content:space-between; width:100%; margin-top:-2px; }
.sr-risk__ends span{ font-family:var(--font-mono); font-size:11px; font-weight:700; color:var(--text-faint);
  background:var(--surface-inset); border-radius:var(--radius-pill); padding:3px 9px; }
.sr-risk__sub{ font-size:13px; color:var(--text-muted); margin-top:8px; }
`;
  ...
  return (
    <div className="sr-risk" style={{ width: size }}>
      <svg className="sr-risk__svg" width={size} height={h} viewBox={`0 0 ${size} ${h}`} role="img" aria-label={`${label} ${value} of ${max}`}>
        ...
        <path className="sr-risk__track" fill="none" strokeWidth="16" strokeLinecap="round" d={arcPath} />
        <path className="sr-risk__glow" fill="none" stroke={color} strokeWidth="16" strokeLinecap="round"
              strokeDasharray={dash} strokeDashoffset={offset} filter={`url(#${glowId})`}
