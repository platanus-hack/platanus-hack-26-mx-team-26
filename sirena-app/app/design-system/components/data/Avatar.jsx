...
const CSS = `
.sr-avatar{ display:inline-flex; align-items:center; justify-content:center; border-radius:50%; font-family:var(--font-body); font-weight:700; color:#fff; flex:none; position:relative; overflow:visible; background:var(--coral-500); }
...
export function Avatar({ name = '', src, size = 'md', status, ring = false, ringColor, style, ...rest }) {
  const showRing = ring || !!ringColor;
  const cls = ['sr-avatar', `sr-avatar--${size}`, showRing && 'sr-avatar__ring'].filter(Boolean).join(' ');
  const ringStyle = showRing && ringColor ? { boxShadow: `0 0 0 2px var(--surface), 0 0 0 3px ${ringColor}` } : null;
  return (
    <span className={cls} style={{ background: src ? undefined : pick(name), ...ringStyle, ...style }} title={name} {...rest}>
